...
def run_randomized_quantile_interval_experiment(
    ...
    X_on, Y_on, mu_on, c_on, w_i = generate_data(n_online, seed)
    ...
    for t in range(n_online):
        ...
        if not sel:
            continue
        selected += 1
        interval = construct_prediction_interval(t, X_on[:t+1], Y_on[:t+1], mu_on[:t+1], c_on[:t+1], w_i[:t+1], M, alpha, quantile_method=quantile_method)
        iscov = is_covered(Y_on[t], interval[:4])
        bound_1, bound_2, bound_3, bound_4 = interval[:4] #interval这个tuple的前四个元素是bound_1, bound_2, bound_3, bound_4
        length = (bound_2 - bound_1) + (bound_4 - bound_3)
        is_inf = not np.isfinite(length)
        cal_size = interval[4] #interval这个tuple的第5个元素是cal_size

...
def run_randomized_quantile_interval_experiment(
    ...
    for t in range(n_online):
        ...
        interval = construct_prediction_interval(
            t, X_on, Y_on, mu_on, X_off, Y_off, mu_off, M, alpha, quantile_method
        )
        iscov = is_covered(Y_on[t], interval[:2])
        lower, upper = interval[:2]  # 前两个元素是 lower、upper
        length = upper - lower
        is_inf = not np.isfinite(length)
        cal_size = interval[2]       # 第三个元素是 cal_size
...
def main():
    ...
    # 并行进程数：M2 建议先 6 试试（避免过度并行 + BLAS 多线程）
    max_workers = min(6, os.cpu_count() or 4)

    # 外层总体进度条：方法×n_online 组数
    ...
    # ---------------- 从 CSV 读回并复用原有的统计与画图逻辑 ----------------
    df = pd.read_csv(csv_path)

    # 保持与原逻辑一致：只输出第一个 n_online 的详细表格与曲线
    ...
    for method_name in ["upper", "randomize"]:
        ...
        # 统计每个 t 的 selection conditional 指标（与原版一致）
        t_to_cov = {}
        t_to_cov_count = {}
        t_to_len = {}
        t_to_len_count = {}

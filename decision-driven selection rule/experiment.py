...
from config import M, alpha #, selection_rule_key, interval_method_key
...
#     M: int = M,
#     alpha: float = alpha,
#     selection_rule_name: str = selection_rule_key,
#     interval_method_name: str = interval_method_key
# ) -> Tuple[float, int]:
#     X_on, Y_on, mu_on = generate_data(n_online)
#     coverage = []
#     history = []
#     selected = 0

#     # sel_rule = get_selection_rule(selection_rule_name)
#     # interval_func = get_interval_constructor(interval_method_name)

#     for t in range(n_online):
#         if selection_rule_name == "decision_driven":
#             sel = sel_rule(X_on[t, 0], sum(history))
#         elif selection_rule_name == "symmetric":
#             sel = sel_rule(mu_on[t], mu_on[:t].tolist())
#         else:
#             raise ValueError(f"Unrecognized selection rule: {selection_rule_name}")

#         history.append(sel)
#         if not sel:
#             continue
#         selected += 1
#         interval = interval_func(t, X_on, Y_on, mu_on, M, alpha, sel_rule)
#         coverage.append(is_covered(Y_on[t], interval))

#     if selected == 0:
#         return float('nan'), 0
#     return float(np.mean(coverage)), selected
...
def run_randomized_quantile_interval_experiment(
    ...
    for t in range(n_online):
        sel = selection_rule(X_on[t, 0], sum(history))
        history.append(sel)
        if not sel:
            continue
        selected += 1
...
def main():
    ...
    # 打印表格
    # header = ("n_online", "AvgSel", "CovMean", "CovStd", "LenMean", "LenStd", "InfFracMean", "InfFracStd", "CalMean", "CalStd")
    # print("{:>8} | {:>7} | {:>7} | {:>7} | {:>7} | {:>7} | {:>11} | {:>11} | {:>8} | {:>8}".format(*header))
    # print("-"*90)
    # for n_online, t_vals, t_covs, t_lens, t_inf_fracs, t_cals, avg_sel in summary:
    #     cov_mean = np.nanmean(t_covs)
    #     cov_std  = np.nanstd(t_covs)
    #     len_mean = np.nanmean(t_lens)
    #     len_std  = np.nanstd(t_lens)
    #     inf_mean = np.nanmean(t_inf_fracs)
    #     inf_std  = np.nanstd(t_inf_fracs)
    #     cal_mean = np.nanmean(t_cals)
    #     cal_std  = np.nanstd(t_cals)
    #     print(f"{n_online:8d} | {avg_sel:7.1f} | {cov_mean:7.3f} | {cov_std:7.3f} | {len_mean:7.3f} | {len_std:7.3f} | {inf_mean:11.3f} | {inf_std:11.3f} | {cal_mean:8.3f} | {cal_std:8.3f}")

...
def run_randomized_quantile_interval_experiment(
    ...
    for t in range(n_online):
        ...
        if not sel:
            continue
        selected += 1
        interval = construct_prediction_interval(t, X_on, Y_on, mu_on, M, alpha, quantile_method)
...
def main():
    ...
    # 分别运行两种方法
    for method in ["upper", "randomize"]:
        print(f"\nRunning experiments for {method} quantile method...")
        ...
        for n_online in n_online_list:
            ...
            summary_upper.append((n_online, t_vals, t_covs, t_lens, t_inf_fracs, t_cals, avg_sel)) if method == "upper" else summary_randomize.append((n_online, t_vals, t_covs, t_lens, t_inf_fracs, t_cals, avg_sel))

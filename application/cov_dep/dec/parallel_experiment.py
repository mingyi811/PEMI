...
import csv
from config import M, alpha, reference_set_method  
...
def run_randomized_quantile_interval_experiment(
    ...
    for t in range(n_online):
        sel = selection_rule_final(mu_on[t], sum(history))
        history.append(sel)
        if not sel:
            continue
        selected += 1
        interval = construct_prediction_interval(
            t, Y_on, mu_on, Y_off, mu_off, history, M, alpha, quantile_method, reference_set_method
        )
...
def main():
    ...
    # Generate timestamped filenames and directories
    ...
    # Number of parallel workers (limit to 6 to avoid over-parallelization and BLAS multi-threading)
            ...
            with ProcessPoolExecutor(max_workers=max_workers) as ex:
                ...
                with open(csv_path, "a", newline="") as fcsv:
                    ...
                    for fut in as_completed(futures):
                        ...
                        if len(t_records) == 0:
                            # Placeholder row for selected_total statistics
                            writer.writerow([
                                method, n_online, run_id, -1,
                                "", "", "", "",
                                selected
                            ])
                        else:
    ...
    for method_name in ["upper", "randomize"]:
        df_m = df[(df["method"] == method_name) & (df["n_online"] == n_online)]

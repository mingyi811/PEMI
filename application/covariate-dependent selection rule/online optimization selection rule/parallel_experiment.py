...
def run_randomized_quantile_interval_experiment(
    ...
    """
    #X_on, Y_on, mu_on_linear, mu_on_random_forest, mu_on_polynomial = generate_data(n_online, seed)
    #X_off, Y_off, mu_off_linear, mu_off_random_forest, mu_off_polynomial = generate_data(n_offline, seed)
    Y_all, muhat_1, muhat_2, muhat_3 = generate_data(n_online+n_offline, seed)
    Y_on = Y_all[:n_online]
    Y_off = Y_all[n_online:]
    mu_on_1 = muhat_1[:n_online]
    mu_on_2 = muhat_2[:n_online]
    mu_on_3 = muhat_3[:n_online]
    mu_off_1 = muhat_1[n_online:]
    mu_off_2 = muhat_2[n_online:]
    mu_off_3 = muhat_3[n_online:]
    t_records = []
    selected = 0
    for t in range(n_online):
        sel = selection_rule_optimization(mu_on_1[:t+1].tolist(), mu_on_2[:t+1].tolist(), mu_on_3[:t+1].tolist(), method=method)
        if not sel:
            continue
        selected += 1
        interval = construct_prediction_interval(
            t, Y_on, mu_on_1[:t+1], mu_on_2[:t+1], mu_on_3[:t+1], Y_off, mu_off_1, mu_off_2, mu_off_3, M, alpha, quantile_method=quantile_method,reference_set_method=reference_set_method
        )
...
def main():
    ...
    for quantile_method in ["upper", "randomize"]:
        ...
        for n_online in n_online_list:
            ...
            with ProcessPoolExecutor(max_workers=max_workers) as ex:
                ...
                # Inner progress bar for this group
                with open(csv_path, "a", newline="") as fcsv:
                    ...
                    for fut in as_completed(futures):
                        ...
                        if len(t_records) == 0:
                            # Placeholder row for runs with no selected t (for selected_total statistics)
                            writer.writerow([
    ...
    # Output detailed table and plot only for the first n_online (consistent with original logic)
    ...
    for mth in ["upper", "randomize"]:
        ...
        # Assemble all_t_records_* list structure
        all_t_records = [
            (int(row.t),
             bool(row.is_covered),
             row.length,
        ...
        # Compute selection conditional metrics for each t (consistent with original)
        t_to_cov = {}
        t_to_cov_count = {}
        t_to_len = {}
        t_to_len_count = {}

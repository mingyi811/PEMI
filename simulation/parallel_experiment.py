...
def run_randomized_quantile_interval_experiment(
    ...
    返回：
      ...
      - selected: The number of t selected by this run
    """

    X_all, Y_all, mu_all = generate_data(n_online+n_offline, sigma, seed, model_type)
    Y_on = Y_all[:n_online]
    mu_on = mu_all[:n_online]
    Y_off = Y_all[n_online:]
    X_off = X_all[n_online:]
    ...
    for t in range(n_online):
        ...
        interval = construct_prediction_interval(
            t, Y_on, mu_on, Y_off, mu_off, X_off, X_on, M, alpha, quantile_method=quantile_method,
            reference_set_method=reference_set_method, qrf_model=qrf_model
        )
...
def _worker_one_run(run_id: int, n_online: int, n_offline: int, quantile_method: str, base_seed: int, reference_set_method: str, qrf_model):
    """
...
def main():
    ...
    # Different number of rows to test (consistent)
    n_online_list = [200]
    n_offline = 0
    n_runs = 10000
    ...
    n_pretrain_qrf = 1000
    pretrain_seed = 42
    pretrain_X, pretrain_Y, pretrain_mu = generate_data(n_pretrain_qrf, sigma, pretrain_seed, model_type)
    qrf_model = RandomForestQuantileRegressor(n_estimators=100, random_state=pretrain_seed)
    qrf_model.fit(pretrain_X, pretrain_Y)
    ...
    # Outer overall progress bar: method×n_online combinations
    ...
    # Read from CSV and reuse original statistics and plotting logic
